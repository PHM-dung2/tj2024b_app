/*
  ...
  상태(state/데이터)가 있는 위젯 : StatefulWidget, 재 렌더링(새로고침), 동적 UI
    1.한번 출력된 화면은 불면(고정)이다. setState() 이용하여 재렌더링이 된다.
    2. 반드시 상태를 관리하는 별도의 클래스가 필요하다.
      -> 한번 그려낸 화면을 상태 변화에 따라 다시 그려낼 수 있다.
    3. 사용법
      class 상태관리위젯명 extends StatefulWidget{
        상태를사용할위젯명2 createState() => 상태를사용할위젯명2();
      }
      class 새로운위젯명 extends State< 상태관리위젯명 > {
        @override
        Widget build( BuildContext context ){
          return 위젯명( );
        }
    ...
    4. 대표적인 주요 함수
      initState( ) : 위젯이 처음에 생성될 때 실행되는 초기화 함수  [ 위젯 탄생 ]
      setState( )  : 상태를 변경하고 화면을 다시 렌더링하는 함수   [ 위젯 업데이트 ]
      dispose( )   : 위젯이 제거될 떄 실행되는 함수              [ 위젯 사망 ]
*/
...
class MyApp1 extends StatefulWidget{
  MyApp1State createState() => MyApp1State(); // * 상태사용할 위젯과 연결, createState()
}
...
class MyApp1State extends State< MyApp1 >{
  ...
  void increment(){ // 값 증가 함수
    // count++; print( count );
    // 상태 변화에 따른 재 렌더링, setState( (){} )
    setState(() {
      count++;
    });
  } // f end

  // 상태위젯이 최초로 실행될 때 딱 1번 실행되는 함수, initState
  @override
  void initState(){
    print("상태 위젯이 최초 1번 실행하는 함수입니다.");
  }
